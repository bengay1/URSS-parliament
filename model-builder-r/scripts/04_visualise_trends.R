...
if (file.exists("data/combined_data_raw.csv")) {
  ...
  p_party <- ggplot(party_trends,
    ...
    labs(
      ...
      x = "Year",
      y = "Average Ideology Score (Right ← → Left)",
      color = "Party",
      caption = "Positive scores = leftward | Negative scores = rightward"
    ) +
    scale_color_manual(
      values = c(
        "Conservative" = "#0087DC",
...
featured_mps <- c("Keir Starmer", "David Cameron", "Theresa May",

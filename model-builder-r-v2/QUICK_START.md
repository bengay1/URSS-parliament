...
# Top 10 rightward-oriented MPs in 2024
all_scores %>%
  filter(year == 2024) %>%
  arrange(desc(z_score)) %>%
  head(10)

# Top 10 leftward-oriented MPs in 2024
all_scores %>%
  filter(year == 2024) %>%
  arrange(z_score) %>%
  head(10)
...
# MPs shifting rightward most significantly
mp_slopes %>%
  filter(p_value < 0.05, slope > 0) %>%
  arrange(desc(slope))

# MPs shifting leftward most significantly
mp_slopes %>%
  filter(p_value < 0.05, slope < 0) %>%
  arrange(slope)
```
